...
(class-ns APersistentVector [AFn Associative #_"Comparable" Counted IFn Hashed ILookup Indexed IObject IPersistentCollection IPersistentStack IPersistentVector Reversible Seqable Sequential]
    ...
    (defm APersistentVector IFn
        (#_"Object" IFn'''invoke [#_"APersistentVector" this, #_"Object" key]
            (when (integer? key) => (throw! "key must be integer")
                (nth this (.intValue key))
            )
        )
    )
    ...
    (defm APersistentVector ILookup
        (#_"Object" ILookup'''valAt
            ...
            ([#_"APersistentVector" this, #_"Object" key, #_"Object" not-found]
                (when (integer? key) => not-found
                    (let-when [#_"int" i (.intValue key)] (< -1 i (count this)) => not-found
                        (nth this i)
                    )
                )
            )
    ...
    #_foreign
    (§ defm APersistentVector #_"Comparable"
        (#_"int" Comparable'''compareTo [#_"APersistentVector" this, #_"IPersistentVector" that]
...
(class-ns SubVector [#_"APersistentVector" Associative #_"Comparable" Counted IFn Hashed ILookup IMeta Indexed IObj IObject IPersistentCollection IPersistentStack IPersistentVector Reversible Seqable Sequential]
    ...
    (defm SubVector Indexed
        (#_"Object" Indexed'''nth
            ...
            ([#_"SubVector" this, #_"int" i, #_"Object" not-found]
...
(class-ns VNode []
    (defn #_"VNode" VNode'new [#_"Thread'" edit, #_"Object[]" a]
        (merge (VNode'class.)
            (hash-map
                #_"Thread'" :edit edit
                #_"Object[]" :a a
    ...
    (def #_"VNode" VNode'EMPTY (VNode'new (atom nil), (object-array 32)))
    ...
    (defn #_"VNode" VNode''doAssoc [#_"VNode" this, #_"int" level, #_"int" i, #_"Object" o]
        (let [
            #_"Object[]" a (aclone (:a this))
            _
                (if (zero? level)
                    (aset a (bit-and i 0x1f) o)
                    (aswap a (bit-and (unsigned-bit-shift-right i level) 0x1f) VNode''doAssoc (- level 5), i, o)
                )
        ]
            (VNode'new (:edit this), a)
        )
    )
)
...
(class-ns TransientVector [AFn Counted IFn ILookup Indexed ITransientAssociative ITransientCollection ITransientVector]
        ...
        (when (< -1 i (:cnt this)) => (throw! "index is out of bounds")
            (when (< i (TransientVector''tailoff this)) => (:tail this)
                (loop-when-recur [#_"VNode" node (:root this) #_"int" level (:shift this)]
                                 (pos? level)
                                 [(aget (:a node) (bit-and (unsigned-bit-shift-right i level) 0x1f)) (- level 5)]
                              => (:a node)
                )
            )
        )
    ...
    (defn- #_"Object[]" TransientVector''editableArrayFor [#_"TransientVector" this, #_"int" i]
        (when (< -1 i (:cnt this)) => (throw! "index is out of bounds")
            (when (< i (TransientVector''tailoff this)) => (:tail this)
                (loop-when-recur [#_"VNode" node (:root this) #_"int" level (:shift this)]
                                 (pos? level)
                                 [(TransientVector''ensureEditable this, (aget (:a node) (bit-and (unsigned-bit-shift-right i level) 0x1f))) (- level 5)]
                              => (:a node)
                )
            )
        )
    ...
    (defm TransientVector Indexed
        (#_"Object" Indexed'''nth
            ([#_"TransientVector" this, #_"int" i]
                (VNode''assertEditable (:root this))
                (aget (TransientVector''arrayFor this, i) (bit-and i 0x1f))
            )
    ...
    (defm TransientVector IFn
        (#_"Object" IFn'''invoke [#_"TransientVector" this, #_"Object" arg]
            (when (integer? arg) => (throw! "arg must be integer")
                (Indexed'''nth this (.intValue arg))
            )
        )
    )
    ...
    (defm TransientVector ITransientAssociative
        ...
        (#_"IMapEntry" ITransientAssociative'''entryAt [#_"TransientVector" this, #_"Object" key]
            (let [
                #_"Object" val (get this key TransientVector'NOT_FOUND)
            ]
                (when-not (identical? val TransientVector'NOT_FOUND)
    ...
    (defn- #_"VNode" TransientVector''pushTail [#_"TransientVector" this, #_"int" level, #_"VNode" parent, #_"VNode" tail-node]
        ;; if parent is leaf, insert node,
        ;; else does it map to an existing child? -> nodeToInsert = pushNode one more level
        ;; else alloc new path
        ;; return nodeToInsert placed in parent
        (let [
            parent (TransientVector''ensureEditable this, parent)
            #_"int" x (bit-and (unsigned-bit-shift-right (dec (:cnt this)) level) 0x1f)
            #_"VNode" child
                (when-not (= level 5) => tail-node
                    (let [
                        child (aget (:a parent) x)
                    ]
                        (if (some? child)
                            (TransientVector''pushTail this, (- level 5), child, tail-node)
                            (VNode''newPath tail-node, (:edit (:root this)), (- level 5))
                        )
                    )
                )
            _ (aset (:a parent) x child)
        ]
            parent
        )
    )
    ...
    (defm TransientVector ITransientCollection
        (#_"TransientVector" ITransientCollection'''conj [#_"TransientVector" this, #_"Object" val]
            (VNode''assertEditable (:root this))
            (if (< (- (:cnt this) (TransientVector''tailoff this)) 32) ;; room in tail?
                (let [
                    _ (aset (:tail this) (bit-and (:cnt this) 0x1f) val)
                ]
                    (-> this (update :cnt inc))
                )
                ;; full tail, push into tree
                (let [
                    #_"VNode" tail-node (VNode'new (:edit (:root this)), (:tail this))
                    this (assoc this :tail (object-array 32))
                    _ (aset (:tail this) 0 val)
                ]
                    (if (< (bit-shift-left 1 (:shift this)) (unsigned-bit-shift-right (:cnt this) 5)) ;; overflow root?
                        (let [
                            #_"Object[]" a
                                (doto (object-array 32)
                                    (aset 0 (:root this))
                                    (aset 1 (VNode''newPath tail-node, (:edit (:root this)), (:shift this)))
                                )
                            #_"VNode" root (VNode'new (:edit (:root this)), a)
                        ]
                            (-> this (assoc :root root) (update :shift + 5) (update :cnt inc))
                        )
                        (let [
                            #_"VNode" root (TransientVector''pushTail this, (:shift this), (:root this), tail-node)
                        ]
    ...
    (defn- #_"VNode" TransientVector''doAssoc [#_"TransientVector" this, #_"int" level, #_"VNode" node, #_"int" i, #_"Object" val]
        (let [
            node (TransientVector''ensureEditable this, node)
        ]
            (if (zero? level)
                (aset (:a node) (bit-and i 0x1f) val)
                (let [
                    #_"int" x (bit-and (unsigned-bit-shift-right i level) 0x1f)
                ]
                    (aset (:a node) x (TransientVector''doAssoc this, (- level 5), (aget (:a node) x), i, val))
                )
            )
            node
        )
        ...
        (#_"TransientVector" ITransientVector'''assocN [#_"TransientVector" this, #_"int" i, #_"Object" val]
            ...
            (if (< -1 i (:cnt this))
                (if (<= (TransientVector''tailoff this) i)
                    (do
                        (aset (:tail this) (bit-and i 0x1f) val)
                        this
                    )
                    (do
    ...
    (defn- #_"VNode" TransientVector''popTail [#_"TransientVector" this, #_"int" level, #_"VNode" node]
        (let [
            node (TransientVector''ensureEditable this, node)
            #_"int" x (bit-and (unsigned-bit-shift-right (- (:cnt this) 2) level) 0x1f)
        ]
            (cond
                (< 5 level)
                    (let [
                        #_"VNode" child (TransientVector''popTail this, (- level 5), (aget (:a node) x))
                    ]
    ...
    (defm TransientVector ITransientVector
        (#_"TransientVector" ITransientVector'''pop [#_"TransientVector" this]
            ...
            (cond
                ...
                (or (= (:cnt this) 1) (not (zero? (bit-and (dec (:cnt this)) 0x1f))))
                    (-> this (update :cnt dec))
                :else
                    (let [
                        #_"Object[]" tail (TransientVector''editableArrayFor this, (- (:cnt this) 2))
                        #_"VNode" root (:root this)
                        root (or (TransientVector''popTail this, (:shift this), root) (VNode'new (:edit root), (object-array 32)))
                        this
                            (if (and (< 5 (:shift this)) (nil? (aget (:a root) 1)))
                                (-> this
                                    (update :shift - 5)
                                    (assoc :root (TransientVector''ensureEditable this, (aget (:a root) 0)))
                                )
                                (-> this
                                    (assoc :root root)
                                )
    ...
    (defm TransientVector ITransientCollection
        (#_"PersistentVector" ITransientCollection'''persistent [#_"TransientVector" this]
            ...
            (let [
                #_"Object[]" trimmed-tail (object-array (- (:cnt this) (TransientVector''tailoff this)))
                _ (System/arraycopy (:tail this), 0, trimmed-tail, 0, (alength trimmed-tail))
            ]
                (PersistentVector'new (:cnt this), (:shift this), (:root this), trimmed-tail)
            )
        )
    )
)
...
(class-ns PersistentVector [#_"APersistentVector" Associative #_"Comparable" Counted IEditableCollection IFn Hashed IKVReduce ILookup IMeta Indexed IObj IObject IPersistentCollection IPersistentStack IPersistentVector IReduce Reversible Seqable Sequential]
    ...
    (def #_"PersistentVector" PersistentVector'EMPTY (PersistentVector'new 0, 5, VNode'EMPTY, (object-array 0)))

    (defn #_"PersistentVector" PersistentVector'adopt [#_"Object[]" items]
        (PersistentVector'new (alength items), 5, VNode'EMPTY, items)
    )
        ...
        (let [
            ...
            (cond
                (some? s) ;; >32, construct with array directly
                    (into (PersistentVector'new 32, 5, VNode'EMPTY, a) s)
                (= i 32) ;; exactly 32, skip copy
                    (PersistentVector'new 32, 5, VNode'EMPTY, a)
                :else ;; <32, copy to minimum array and construct
                    (let [
                        #_"Object[]" b (object-array i)
                        _ (System/arraycopy a, 0, b, 0, i)
                    ]
                        (PersistentVector'new i, 5, VNode'EMPTY, b)
                    )
            )
        )
    )
    ...
    (defm PersistentVector Indexed
        (#_"Object" Indexed'''nth
            ([#_"PersistentVector" this, #_"int" i]
                (aget (PersistentVector''arrayFor this, i) (bit-and i 0x1f))
            )
    ...
    (defn- #_"VNode" PersistentVector''pushTail [#_"PersistentVector" this, #_"int" level, #_"VNode" parent, #_"VNode" tail-node]
        ;; if parent is leaf, insert node,
        ;; else does it map to an existing child? -> nodeToInsert = pushNode one more level
        ;; else alloc new path
        ;; return nodeToInsert placed in copy of parent
        (let [
            #_"Object[]" a (aclone (:a parent))
            #_"int" x (bit-and (unsigned-bit-shift-right (dec (:cnt this)) level) 0x1f)
            #_"VNode" child
                (when-not (= level 5) => tail-node
                    (if-some [child (aget a x)]
                        (PersistentVector''pushTail this, (- level 5), child, tail-node)
                        (VNode''newPath tail-node, (:edit (:root this)), (- level 5))
                    )
                )
            _ (aset a x child)
        ]
            (VNode'new (:edit parent), a)
        )
    )
    ...
    (defm PersistentVector IPersistentCollection
        (#_"PersistentVector" IPersistentCollection'''conj [#_"PersistentVector" this, #_"Object" val]
            (let [
                ...
                (if (< (- (:cnt this) (PersistentVector''tailoff this)) 32) ;; room in tail?
                    (let [
                        #_"Object[]" tail (object-array (inc tail-len))
                        _ (System/arraycopy (:tail this), 0, tail, 0, tail-len)
                        _ (aset tail tail-len val)
                    ]
                        (PersistentVector'new (meta this), (inc (:cnt this)), (:shift this), (:root this), tail)
                    )
                    ;; full tail, push into tree
                    (let [
                        #_"VNode" tail-node (VNode'new (:edit (:root this)), (:tail this))
                        ...
                        [#_"VNode" root shift]
                            (if (< (bit-shift-left 1 shift) (unsigned-bit-shift-right (:cnt this) 5)) ;; overflow root?
                                (let [
                                    #_"Object[]" a
                                        (doto (object-array 32)
                                            (aset 0 (:root this))
                                            (aset 1 (VNode''newPath tail-node, (:edit (:root this)), shift))
                                        )
                                ]
                                    [(VNode'new (:edit (:root this)), a) (+ shift 5)]
                                )
                                [(PersistentVector''pushTail this, shift, (:root this), tail-node) shift]
                            )
                    ]
                        (PersistentVector'new (meta this), (inc (:cnt this)), shift, root, (object-array [ val ]))
                    )
                )
            )
        )
        ...
        (#_"PersistentVector" IPersistentVector'''assocN [#_"PersistentVector" this, #_"int" i, #_"Object" o]
            (if (< -1 i (:cnt this))
                (if (<= (PersistentVector''tailoff this) i)
                    (let [
                        #_"Object[]" tail (object-array (alength (:tail this)))
                        _ (System/arraycopy (:tail this), 0, tail, 0, (alength tail))
                        _ (aset tail (bit-and i 0x1f) o)
                    ]
                        (PersistentVector'new (meta this), (:cnt this), (:shift this), (:root this), tail)
                    )
                    (PersistentVector'new (meta this), (:cnt this), (:shift this), (VNode''doAssoc (:root this), (:shift this), i, o), (:tail this))
                )
                (when (= i (:cnt this)) => (throw! "index is out of bounds")
    ...
    (defn- #_"VNode" PersistentVector''popTail [#_"PersistentVector" this, #_"int" level, #_"VNode" node]
        (let [
            ...
            (cond
                (< 5 level)
                    (let [
                        #_"VNode" child (PersistentVector''popTail this, (- level 5), (aget (:a node) x))
                    ]
                        (when (or (some? child) (pos? x))
                            (VNode'new (:edit (:root this)), (doto (aclone (:a node)) (aset x child)))
                        )
                    )
                (pos? x)
                    (VNode'new (:edit (:root this)), (doto (aclone (:a node)) (aset x nil)))
            )
        )
    )
    ...
    (defm PersistentVector IPersistentStack
        (#_"PersistentVector" IPersistentStack'''pop [#_"PersistentVector" this]
            (condp = (:cnt this)
                ...
                1   (with-meta PersistentVector'EMPTY (meta this))
                (let [
                    #_"int" tail-len (alength (:tail this))
                ]
                    (if (< 1 (- (:cnt this) (PersistentVector''tailoff this)))
                        (let [
                            #_"Object[]" tail (object-array (dec tail-len))
                            _ (System/arraycopy (:tail this), 0, tail, 0, (alength tail))
                        ]
                            (PersistentVector'new (meta this), (dec (:cnt this)), (:shift this), (:root this), tail)
                        )
                        (let [
                            #_"Object[]" tail (PersistentVector''arrayFor this, (- (:cnt this) 2))
                            #_"int" shift (:shift this)
                            #_"VNode" root (or (PersistentVector''popTail this, shift, (:root this)) VNode'EMPTY)
                            [shift root]
                                (when (and (< 5 shift) (nil? (aget (:a root) 1))) => [shift root]
                                    [(- shift 5) (aget (:a root) 0)]
                                )
                        ]
    ...
    (defm PersistentVector IKVReduce
        (#_"Object" IKVReduce'''kvreduce [#_"PersistentVector" this, #_"IFn" f, #_"Object" r]
            (loop-when [r r #_"int" i 0] (< i (:cnt this)) => r
                (let [
                    #_"Object[]" a (PersistentVector''arrayFor this, i)
                    r
                        (loop-when [r r #_"int" x 0] (< x (alength a)) => r
                            (let [
                                r (f r (+ i x) (aget a x))
                            ]
                                (when-not (reduced? r) => r
...
(class-ns WNode []
    ...
    (def #_"WNode" WNode'EMPTY (WNode'new (atom nil), (object-array 32), nil))

    (defn- #_"int[]" WNode'regular-ranges [#_"int" shift, #_"int" cnt]
        (let [
            #_"int" k (<< 1 shift)
            #_"int[]" x (int-array 33)
        ]
    ...
    (defn #_"int" WNode'last-range [#_"int[]" x]
    ...
    (defn- #_"WNode" WNode''last-child [#_"WNode" this]
        (let [
            #_"Object[]" a (:array this) #_"int[]" x (:index this)
        ]
    ...
    (defn- #_"WNode" WNode''remove-leftmost-child [#_"WNode" this]
        (let [
            ...
            (when (some? (aget a 1))
                (let [
                    #_"Object[]" a' (doto (object-array 32) (acopy 0 a 1 31))
                    #_"int[]" x (:index this)
                    #_"int[]" x'
                        (when (some? x)
                            (let [
                                ...
                                (doto x' (aset e 0) (aset 32 e))
                            )
                        )
                ]
                    (WNode'new (:edit this), a', x')
                )
            )
        )
    )
            ...
            #_"Object[]" a'
                (if (some? x)
                    ...
                    (let [
                        ...
                        (doto (object-array 32) (aset 0 node) (acopy 1 a 1 n))
                    )
                )
        ]
            (WNode'new nil, a', x')
        ...
        (let [
            ...
            (if (some? x)
                (let [
                    ...
                    (WNode'new nil, (doto (aclone a) (aset e node)), (doto (aclone x) (aset e (+ (aget x e) delta))))
                )
                (let [
                    #_"int" m (dec (WNode'index-of-nil a))
                    x (:index node)
                ]
                    (when (some? x) => (WNode'new nil, (doto (aclone a) (aset m node)), nil)
                        (let [
                            #_"int" k (<< 1 shift)
                            #_"int[]" x' (int-array 33)
                            #_"int" n (aget x (dec (aget x 32)))
                            ;; %% almost WNode'regular-ranges
                            x'
                                (loop [#_"int" j 0 #_"int" i k]
                                    (if (< j m)
                                        (do
                            ...
                            (WNode'new nil, (doto (object-array 32) (acopy 0 a 0 m) (aset m node)), x')
                        )
                    )
                )
            )
            ...
            (when (or (< m 32) (and (some? tail-node) (< 5 shift)))
                (let [
                    ...
                    #_"int[]" x'
                        (when (or (some? x) (< n 32))
                            (let [
                                x' (if (some? x) (aclone x) (WNode'regular-ranges shift, cnt))
                            ]
                                (if (and (some? tail-node) (< 5 shift))
                                    ...
                                    (do
                                        ...
                                        (doto x' (aset 32 (inc m)))
                                    )
                                )
                            )
                        )
                    #_"Object[]" a' (doto (object-array 32) (acopy 0 a 0 m))
                ]
                    (if (some? tail-node)
    ...
    (defn #_"WNode" WNode''slice-right [#_"WNode" this, #_"int" shift, #_"int" end]
        ...
        (when (pos? shift) => (WNode'new nil, (doto (value-array end) (acopy 0 (:array this) 0 end)), nil)
            (let [
                #_"Object[]" a (:array this) #_"int[]" x (:index this)
                ...
                #_"int[]" y (:index child)
                #_"Object[]" a' (doto (object-array 32) (acopy 0 a 0 m) (aset m child))
                #_"int[]" x'
                    (when (or (some? x) (some? y))
                        (let [
                            ...
                            (aset x' m (+ (if (pos? m) (aget x' (dec m)) 0) delta))
                            (doto x' (aset 32 (inc m)))
                        )
                    )
            ]
    ...
    (defn #_"WNode" WNode''slice-left [#_"WNode" this, #_"int" shift, #_"int" start, #_"int" end]
        (if (zero? shift)
            ...
            (let [
                ...
                (WNode'new nil, (doto (value-array n) (acopy 0 a start n)), nil)
            )
            (let [
                #_"Object[]" a (:array this) #_"int[]" x (:index this)
                ...
                (when (pos? n)
                    (let [
                        ...
                        _ (aset x' 32 n)
                        #_"Object[]" a' (doto (object-array 32) (acopy 0 a (if (nil? child) (inc m) m) n))
                        _
                            (when (some? child)
                                (aset a' 0 child)
                            )
            ...
            (let [
                #_"int[]" x'
                    (when (some? (:index this))
                        (doto (int-array 33) (aset 0 (WNode'last-range (:index this))) (aset 32 1))
                    )
            ]
                (recur (WNode'new (:edit this), (doto (object-array 32) (aset 0 this)), x') (+ 5 from) to)
            )
        )
    )
    ...
    (defn- #_"int" WNode''subtree-branch-count [#_"WNode" this, #_"int" shift]
        ...
        (let [
            ...
            (loop-when-recur [#_"int" i 0 #_"int" n 0]
                             ...
                             [(inc i) (+ n (WNode''slot-count (aget a i), (- shift 5)))]
                          => n
            )
        )
    )
    ...
    (defn- #_"[WNode WNode int]" WNode'rebalance-leaves [#_"WNode" n1, #_"int" cnt1, #_"WNode" n2, #_"int" cnt2, #_"int" delta]
        (let [
            ...
            (cond
                ...
                :else
                    (let [
                        ...
                        #_"WNode" n1' (WNode'new nil, a1', nil)
                        #_"WNode" n2' (WNode'new nil, a2', nil)
                    ]
                        (loop [#_"int" i 0 #_"seq" bs (partition-all 32 (concat (WNode''leaf-seq n1) (WNode''leaf-seq n2)))]
                            (when-first [block bs]
                                (let [
                                    ...
                                    (if (< i 32)
                                        (aset a1' i (WNode'new nil, a, nil))
                                        (aset a2' (- i 32) (WNode'new nil, a, nil))
                                    )
                                    (recur (inc i) (next bs))
                                )
                            )
        ...
        (let [
            f'cseq
                (fn [#_"WNode" this #_"int" cnt]
                    (let [
                        #_"int[]" x (or (:index this) (WNode'regular-ranges (- shift 5), cnt))
                        #_"int" n (aget x 32)
                    ]
                        (take n (map list (:array this) (map - x (cons 0 x))))
                    )
                )
            #_"int[]" x (or (:index this) (WNode'regular-ranges shift, cnt))
            #_"int" n (aget x 32)
        ]
    ...
    (defn- #_"[WNode WNode int]" WNode'rebalance [#_"int" shift, #_"WNode" n1, #_"int" cnt1, #_"WNode" n2, #_"int" cnt2, #_"int" delta]
        (when (some? n2) => [n1 nil delta]
            (let [
                ...
                (cond
                    ...
                    :else
                        (let [
                            #_"Object[]" a1' (object-array 33) #_"int[]" b1' (int-array 33) #_"WNode" n1' (WNode'new nil, a1', nil)
                            #_"Object[]" a2' (object-array 33) #_"int[]" b2' (int-array 33) #_"WNode" n2' (WNode'new nil, a2', nil)
                            delta
                                (loop [delta delta #_"int" i 0 #_"seq" bs (partition-all 32 (concat (WNode''child-seq n1, shift, cnt1) (WNode''child-seq n2, shift, cnt2)))]
                                    (when-first [block bs] => delta
                                        (let [
                                            #_"Object[]" a (object-array 33)
                                            #_"int[]" r (int-array 33)
                                            _ (aset a 32 r)
                                            _ (aset r 32 (count block))
                                            _
                                            ...
                                            delta
                                                (when (and (< i 32) (< sbc1 (+ (* i 32) (count block)))) => delta
                                                    (let [
                                            ...
                                            #_"int" k (mod i 32)
                                            _ (aset a' k (WNode'new nil, a, nil))
                                            _ (aset r' k (+ (aget r (dec (aget r 32))) (if (pos? k) (aget r' (dec k)) 0)))
                                            _ (aset r' 32 (inc k))
                                        ]
                                            (recur delta (inc i) (next bs))
                                        )
                                    )
                                )
                            _ (aset a1' 32 b1')
    ...
    (defn #_"[WNode WNode int]" WNode'zippath [#_"int" shift, #_"WNode" n1, #_"int" cnt1, #_"WNode" n2, #_"int" cnt2, #_"int" delta]
        (if (= shift 5)
            (WNode'rebalance-leaves n1, cnt1, n2, cnt2, delta)
            (let [
                ...
                [#_"WNode" c1' #_"WNode" c2' #_"int" d'] (WNode'zippath (- shift 5), c1, ccnt1, c2, ccnt2, 0)
            ]
                (WNode'rebalance shift,
                    (if (identical? c1 c1') n1 (WNode''replace-rightmost-child n1, shift, c1', d')),
                    (+ cnt1 d'),
                    (if c2' (if (identical? c2 c2') n2 (WNode''replace-leftmost-child n2, shift, cnt2, c2', d')) (WNode''remove-leftmost-child n2)),
                    (- cnt2 d'),
                    (+ delta d')
                )
            )
    ...
    (defn #_"[WNode WNode]" WNode'squash-nodes [#_"int" shift, #_"WNode" n1, #_"int" cnt1, #_"WNode" n2, #_"int" cnt2]
        (let [
            #_"Object[]" a1 (:array n1) #_"int" m1 (WNode'index-of-nil a1)
            #_"Object[]" a2 (:array n2) #_"int" m2 (WNode'index-of-nil a2)
            #_"seq" slots (concat (take m1 a1) (take m2 a2))
        ]
            (when (<= (count slots) 32) => [n1 n2]
                (let [
                    #_"seq" s1 (take m1 (or (:index n1) (WNode'regular-ranges shift, cnt1)))
                    #_"seq" s2 (take m2 (or (:index n2) (WNode'regular-ranges shift, cnt2)))
    ...
    (defn #_"WNode" WNode''ensureEditable [#_"WNode" this, #_"Thread'" edit, #_"int" shift]
        (when-not (identical? (:edit this) edit) => this
            (let [
                ...
                (WNode'new edit, (aclone a), (when (and (pos? shift) (some? x)) (aclone x)))
            )
        )
    )

    (defn #_"WNode" WNode''editableRoot [#_"WNode" this]
        (WNode'new (atom (Thread/currentThread)), (aclone (:array this)), (:index this))
    )
    ...
    (defn #_"WNode" WNode'pushTail [#_"WNode" this, #_"int" shift, #_"int" cnt, #_"Thread'" edit, #_"WNode" tail-node]
        (let [
            #_"Object[]" a (aclone (:array this)) #_"int[]" x (:index this)
            x
                (if (some? x)
                    (let [
                        #_"int" e (dec (aget x 32))
                        #_"WNode" child
                            (when (< 5 shift)
                                (let [
                                    ...
                                    (when-not (= n (<< 1 shift))
                                        (WNode'pushTail (aget a e), (- shift 5), (inc n), edit, tail-node)
                                    )
                                )
                            )
                        x (aclone x)
                        ...
                        (if (some? child)
                            ...
                            (let [
                                _ (aset a (inc e) (WNode''newPath tail-node, edit, (- shift 5)))
                                _ (aset x (inc e) (+ (aget x e) 32))
                            ]
                                (doto x (aswap 32 inc))
                            )
                        )
                        ...
                        #_"WNode" child
                            (when (< 5 shift) => tail-node
                                (if-some [child (aget a e)]
                                    (WNode'pushTail child, (- shift 5), cnt, edit, tail-node)
            ...
            (if (some? x)
                (let [
                    ...
                    #_"WNode" child
                        (when (< 5 shift)
                            (let [
                                child (WNode''ensureEditable (aget a e), edit, (- shift 5))
                                #_"int" n (if (pos? e) (- (aget x e) (aget x (dec e))) (aget x 0))
                            ]
                                (when-not (= n (<< 1 shift))
                                    (WNode''pushTail child, (- shift 5), (inc n), edit, tail-node)
                                )
                            )
                        )
                ]
    ...
    (defn #_"WNode" WNode''popTail [#_"WNode" this, #_"int" shift, #_"int" cnt, #_"Thread'" edit]
        (let [
            this (WNode''ensureEditable this, edit, shift)
            #_"Object[]" a (:array this) #_"int[]" x (:index this)
            #_"int" m (& (>> (dec cnt) shift) 0x1f)
        ]
            (if (some? x)
                (let [
                    m (loop-when-recur m (and (pos? (aget x (inc m))) (< m 31)) (inc m) => m)
                ]
                    (cond
                        (< 5 shift)
                            (let [
                                #_"WNode" c (aget a m)
                                #_"WNode" c' (WNode''popTail c, (- m 5), (if (pos? m) (- (aget x m) (aget x (dec m))) (aget x 0)), edit)
                            ]
                                (when (or (some? c') (pos? m))
                                    (let [
                                        #_"int" d
                ...
                (cond
                    (< 5 shift)
                        (let [
                            #_"WNode" c' (WNode''popTail (aget a m), (- shift 5), cnt, edit)
                        ]
                            (when (or (some? c') (pos? m))
                                (let [
                                    _ (aset a m c')
    ...
    (defn #_"WNode" WNode''doAssoc
        ([#_"WNode" this, #_"int" shift, #_"int" i, #_"value" val]
            (let [
                #_"Object[]" a (aclone (:array this)) #_"int[]" x (:index this) #_"int" m (& (>> i shift) 0x1f)
                _
                    (when (pos? shift) => (aset a m val)
                        (let [
                            ...
                            (aswap a m WNode''doAssoc (- shift 5), i, val)
                        )
                    )
            ]
                (WNode'new (:edit this), a, x)
        ...
        ([#_"WNode" this, #_"Thread'" edit, #_"int" shift, #_"int" i, #_"value" val]
            (let [
                this (WNode''ensureEditable this, edit, shift)
                #_"Object[]" a (:array this) #_"int[]" x (:index this) #_"int" m (& (>> i shift) 0x1f)
                _
                    (when (pos? shift) => (aset a m val)
                        (let [
                            [m i]
...
(class-ns TransientWector [AFn Counted IFn ILookup Indexed ITransientAssociative ITransientCollection ITransientVector]
    ...
    (defn- #_"Object[]" TransientWector''arrayFor [#_"TransientWector" this, #_"int" i]
        (when (< -1 i (:cnt this)) => (throw! "index is out of bounds")
            (when (< i (TransientWector''tailoff this)) => (:tail this)
                (loop-when [i i #_"WNode" node (:root this) #_"int" shift (:shift this)] (pos? shift) => (:array node)
                    (let [
                        #_"int[]" x (:index node)
    ...
    (defm TransientWector Indexed
        (#_"value" Indexed'''nth
            ([#_"TransientWector" this, #_"int" i]
                (WNode''assertEditable (:root this))
                ;; %% almost TransientWector''arrayFor
                (when (< -1 i (:cnt this)) => (throw! "index is out of bounds")
                    (let [
                        #_"int" tail-off (- (:cnt this) (alength (:tail this)))
                    ]
                        (when (< i tail-off) => (aget (:tail this) (- i tail-off))
                            (loop-when [i i #_"WNode" node (:root this) #_"int" shift (:shift this)] (pos? shift) => (aget (:array node) (& (>> i shift) 0x1f))
                                (let [
                                    #_"int[]" x (:index node)
                                    #_"int" m (& (>> i shift) 0x1f)
                                    [m i]
        ...
        (#_"TransientWector" ITransientCollection'''conj [#_"TransientWector" this, #_"value" val]
            ...
            (if (< (:tidx this) 32)
                (let [
                    ...
                    (-> this (update :cnt inc) (update :tidx inc))
                )
                (let [
                    #_"WNode" tail-node (WNode'new (:edit (:root this)), (:tail this), nil)
                    this (assoc this :tail (doto (value-array 32) (aset 0 val)), :tidx 1)
                ]
                    (if (WNode''overflow? (:root this), (:shift this), (:cnt this))
                            ...
                            #_"int[]" x'
                                (when (some? (:index (:root this)))
                                    (let [
                                        ...
                                        (doto (int-array 33) (aset 0 n) (aset 1 (+ n 32)) (aset 32 2))
                                    )
                                )
                        ...
                        (let [
                            #_"WNode" root (WNode''pushTail (:root this), (:shift this), (:cnt this), (:edit (:root this)), tail-node)
                        ]
                            (-> this (assoc :root root) (update :cnt inc))
                        )
                    )
            ...
            (cond
                ...
                :else
                    (let [
                        ...
                        #_"WNode" root (WNode''popTail (:root this), (:shift this), (:cnt this), (:edit (:root this)))
                        this
                            (cond
                                (nil? root)
                                    (-> this
    ...
    (defm TransientWector ITransientCollection
        (#_"PersistentWector" ITransientCollection'''persistent [#_"TransientWector" this]
            ...
            (reset! (:edit (:root this)) nil)
            (let [
                #_"values" trimmed-tail (doto (value-array (:tidx this)) (acopy 0 (:tail this) 0 (:tidx this)))
            ]
                (PersistentWector'new nil, (:cnt this), (:shift this), (:root this), trimmed-tail)
            )
        )
    )
)

(class-ns PersistentWector [#_"APersistentVector" Associative #_"Comparable" Counted IEditableCollection IFn Hashed IKVReduce ILookup IMeta Indexed IObj IObject IPersistentCollection IPersistentStack IPersistentVector IPersistentWector IReduce Reversible Seqable Sequential]
    (defn #_"PersistentWector" PersistentWector'new
        ...
        ([#_"IPersistentMap" meta, #_"int" cnt, #_"int" shift, #_"WNode" root, #_"values" tail]
            (merge (PersistentWector'class.) (APersistentVector'new)
                (hash-map
                    #_"IPersistentMap" :_meta meta
                    #_"int" :cnt cnt
                    #_"int" :shift shift
    ...
    (defn- #_"Object[]" PersistentWector''arrayFor [#_"PersistentWector" this, #_"int" i]
        ;; %% almost TransientWector''arrayFor
        (when (< -1 i (:cnt this)) => (throw! "index is out of bounds")
            (when (< i (PersistentWector''tailoff this)) => (:tail this)
                (loop-when [i i #_"WNode" node (:root this) #_"int" shift (:shift this)] (pos? shift) => (:array node)
                    (let [
                        #_"int[]" x (:index node)
                        #_"int" m (& (>> i shift) 0x1f)
    ...
    (defm PersistentWector IPersistentCollection
        (#_"PersistentWector" IPersistentCollection'''conj [#_"PersistentWector" this, #_"value" val]
            (let [
                ...
                (if (< tail-len 32)
                    (let [
                        #_"values" tail (doto (value-array (inc tail-len)) (acopy 0 (:tail this) 0 tail-len) (aset tail-len val))
                    ]
                        ...
                        [#_"WNode" root shift]
                            (if (WNode''overflow? (:root this), shift, (:cnt this))
                                (let [
                                    ...
                                    [(WNode'new (:edit (:root this)), a, nil) (+ shift 5)]
                                )
                                [(WNode'pushTail (:root this), shift, (:cnt this), (:edit (:root this)), tail-node) shift]
                            )
                    ]
                        (PersistentWector'new (:_meta this), (inc (:cnt this)), shift, root, (value-array [ val ]))
                    )
                )
            )
        )
        ...
        (#_"PersistentWector" IPersistentCollection'''empty [#_"PersistentWector" this]
            (PersistentWector'new (:_meta this), 0, 5, WNode'EMPTY, (value-array 0))
        )
    )
                ...
                (let [
                    ...
                    (if (<= tail-off i)
                        (let [
                            ...
                            #_"values" tail (doto (value-array n) (acopy 0 (:tail this) 0 n) (aset (- i tail-off) val))
                        ]
                            (PersistentWector'new (:_meta this), (:cnt this), (:shift this), (:root this), tail)
                        )
                        (PersistentWector'new (:_meta this), (:cnt this), (:shift this), (WNode''doAssoc (:root this), (:shift this), i, val), (:tail this))
                    )
                )
                (when (= i (:cnt this)) => (throw! "index is out of bounds")
    ...
    (defn- #_"WNode" PersistentWector''popTail [#_"PersistentWector" this, #_"int" shift, #_"int" cnt, #_"WNode" node]
        (let [
            ...
            (if (= (alength a) 32)
                (cond
                    (< 5 shift)
                        (let [
                            #_"WNode" child (PersistentWector''popTail this, (- shift 5), cnt, (aget a x))
                        ]
                            (when (or (some? child) (pos? x))
                                (WNode'new (:edit (:root this)), (doto (aclone a) (aset x child)), nil)
                            )
                        )
                    (pos? x)
                        (WNode'new (:edit (:root this)), (doto (aclone a) (aset x nil)), nil)
                )
                (let [
                    #_"int[]" b (aget a 32)
                    x (loop-when-recur x (and (< x 31) (pos? (aget b (inc x)))) (inc x) => x)
                ]
                    (cond
                        (< 5 shift)
                            (let [
                                #_"WNode" c (aget a x)
                                #_"int" n (if (pos? x) (- (aget b x) (aget b (dec x))) (aget b 0))
                                #_"WNode" c' (PersistentWector''popTail this, (- shift 5), n, c)
                            ]
                                (when (or (some? c') (pos? x))
                                    (let [
                                        #_"int" d (if (= (alength (:array c)) 32) 32 (- (WNode'last-range (:index c)) (if (some? c') (WNode'last-range (:index c')) 0)))
                                        b (doto (aclone b) (aswap x - d))
                                        _ (when (nil? c') (aswap b 32 dec))
                                    ]
                                        (WNode'new (:edit (:root this)), (doto (aclone a) (aset x c') (aset 32 b)), nil)
                                    )
                                )
                            )
                        (pos? x)
                            (WNode'new (:edit (:root this)), (doto (aclone a) (aset x nil) (aset 32 (doto (aclone b) (aset x 0) (aswap 32 dec)))), nil)
                    )
                )
            )
        )
    ...
    (defm PersistentWector IPersistentStack
        ;; %% redundant as of APersistentVector
        ...
        (#_"PersistentWector" IPersistentStack'''pop [#_"PersistentWector" this]
            (condp = (:cnt this)
                0   (throw! "can't pop the empty vector")
                1   (PersistentWector'new (:_meta this), 0, 5, WNode'EMPTY, (value-array 0))
                (let [
                    #_"int" tail-len (alength (:tail this))
                ]
                    (if (< 1 tail-len)
                        (let [
                            #_"values" tail (doto (value-array (dec tail-len)) (acopy 0 (:tail this) 0 (dec tail-len)))
                        ]
                            (PersistentWector'new (:_meta this), (dec (:cnt this)), (:shift this), (:root this), tail)
                        )
                        (let [
                            #_"objects" tail (PersistentWector''arrayFor this, (- (:cnt this) 2))
                            #_"int" shift (:shift this)
                            #_"WNode" root (PersistentWector''popTail this, shift, (PersistentWector''tailoff this), (:root this))
                            [shift root]
                                (cond
        ...
        (#_"value" IKVReduce'''kvreduce [#_"PersistentWector" this, #_"IFn" f, #_"value" r]
            (loop-when [r r #_"int" i 0] (< i (:cnt this)) => r
                (let [
                    ...
                    r
                        (loop-when [r r #_"int" x 0] (< x (dec (alength a))) => r
                            (let [
                                r (f r (+ i x) (aget a x))
                            ]
                                (when-not (reduced? r) => r
    ...
    (defm PersistentWector IPersistentWector
            ...
            (cond
                ...
                :else
                    (let [
                        ...
                        (if (<= tail-off start)
                            (let [
                                #_"values" tail (doto (value-array new-cnt) (acopy 0 (:tail this) (- start tail-off) new-cnt))
                            ]
                                ...
                                #_"values" tail
                                    (if tail-cut?
                                        (let [
                                            ...
                                            (doto (value-array n) (acopy 0 (:tail this) 0 n))
                                        )
                                        (PersistentWector''arrayFor (PersistentWector'new nil, new-cnt, (:shift this), root, (value-array 0)), (dec new-cnt))
                                    )
                                root
                                    (when-not tail-cut? => root
                                        (PersistentWector''popTail (PersistentWector'new nil, new-cnt, (:shift this), root, (value-array 0)), (:shift this), new-cnt, root)
                                    )
                            ]
        ...
        (#_"PersistentWector" IPersistentWector'''splicev [#_"PersistentWector" this, #_"PersistentWector" that]
            (let [
                #_"int" c1 (count this) #_"int" c2 (count that)
            ]
                (cond
                    (zero? c1) that
                    ...
                    :else
                        (let [
                            #_"WNode" r1 (:root this) #_"int" s1 (:shift this) #_"Object[]" t1 (:tail this)
                            #_"boolean" o? (WNode''overflow? r1, s1, (+ c1 (- 32 (alength t1))))
                            r1
                                (when o? => (WNode''fold-tail r1, s1, (PersistentWector''tailoff this), t1)
                                    (let [
                                        #_"boolean" regular? (and (= (alength (:array r1)) 32) (= (alength t1) 32))
                                        #_"Object[]" a
                                        ...
                                        _
                                            (when-not regular?
                                                (aset a 32 (doto (int-array 33) (aset 0 (- c1 (alength t1))) (aset 1 c1) (aset 32 2)))
                                            )
                                    ]
                                        (WNode'new nil, a, nil)
                                    )
                            ...
                            s1 (if o? (+ s1 5) s1)
                            #_"WNode" r2 (:root that) #_"int" s2 (:shift that) #_"Object[]" t2 (:tail that)
                            ...
                            (if (some? n2)
                                (let [
                                    #_"int[]" b (doto (int-array 33) (aset 0 c1') (aset 1 (+ c1' c2')) (aset 32 2))
                                    #_"Object[]" a (doto (object-array 33) (aset 0 n1) (aset 1 n2) (aset 32 b))
                                ]
                                    (PersistentWector'new nil, (+ c1 c2), (+ shift 5), (WNode'new nil, a, nil), t2)
                                )
...
(defmacro- gen-vector-method [& s]
    (let [a (with-meta (gensym "a__") {:tag 'objects})]
        `(let [~a (object-array ~(count s))]
            ...
            (PersistentWector'new nil, ~(count s), 5, WNode'EMPTY, ~a)
        )
    )
)
