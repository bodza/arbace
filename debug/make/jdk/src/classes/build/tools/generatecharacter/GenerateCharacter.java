...
public class GenerateCharacter {
    ...
    // bit masks identify each component of a 32-bit property field described above.
    ...
    // bit masks identify the 16-bit priperty field described above, in B table
    ...
    // Can compare masked values with these to determine numeric or lexical types.
    public static int
    ...
    */
    static long buildOne(int c, UnicodeSpec us, SpecialCaseMap[] specialMaps) {
        ...
        if ((us.hasTitleMap() && us.titleMap != us.upperMap) || (bHasUpper && us.hasLowerMap())) {
            resultA |= maskTitleCase;
        }
        if (bHasUpper && !us.hasLowerMap() && !us.hasTitleMap() && verbose) {
            System.out.println("Warning: Character " + hex4(c) + " has upper but " + "no title case; Java won't know this");
        }
        ...
        if (!nobidi) {
            int tmpBidi = (us.bidiCategory > UnicodeSpec.DIRECTIONALITY_OTHER_NEUTRALS || us.bidiCategory == -1) ? maskBidi : (us.bidiCategory << shiftBidi);
            resultA |= tmpBidi;
        }
        ...
        if (identifiers) {
            ...
            if (((resultA & nonzeroJavaPart) != 0) && ((resultA & maskIdentifierInfo) != valueIgnorable)) {
...
OUTER:  for (int i = 0; i < n; i += m) {
    ...
    */
    static void generateCharacterClass(String theTemplateFileName, String theOutputFileName) throws FileNotFoundException, IOException {
        ...
        out.println(commentStart + " This file was generated AUTOMATICALLY from a template file" + commentEnd);
    ...
    */
    static String replaceCommand(String x) {
        ...
        if (x.length() >= 9 && x.substring(0, 7).equals("Lookup(") && x.substring(x.length()-1).equals(")"))
            return genAccess("A", x.substring(7, x.length()-1), (identifiers ? 2 : 32));
        if (x.length() >= 11 && x.substring(0, 9).equals("LookupEx(") && x.substring(x.length()-1).equals(")"))
    ...
    */
    static String genTables() {
        ...
        result.append(commentStart + " The following tables and code generated using:" + commentEnd + "\n  ");
        ...
        if (verbose) {
            System.out.println("The character property tables require " + totalBytes + " bytes.");
        }
    ...
    static void appendEscapedStringFragment(StringBuffer result, char[] line, int length, boolean lastFragment) {
        result.append("    \"");
    ...
    static void addInitializer(String name, String type, int entriesPerChar, int bits, int size) {
        ...
        while ((pos = template.indexOf(commandMarker, pos)) >= 0) {
            int newpos = pos + marklen;
            char ch = 'x';
            while (newpos < template.length() && Character.isJavaIdentifierStart(ch = template.charAt(newpos)) && ch != '_') // Don't allow this in token names
                ++newpos;
    ...
    */
    static void genTable(StringBuffer result, String name, long[] table, int extract, int bits, int size, boolean preshifted, int shift, boolean hexFormat, boolean properties, boolean hexComment) {
    ...
    */
        ...
        for (int j = 0; j < args.length; j++) {
            if (args[j].equals("-verbose") || args[j].equals("-v"))
                ...
                try {
                    ...
                    if (val < 0 || val > 32)
                        FAIL("Incorrect bit field width: " + args[j]);
                    if (sizes == null)
                        sizes = new int[1];
                    else {
    ...
    private static void generateForSizes(long[] map) throws Exception {
        ...
        if ((1 << sum) < map.length || (1 << (sum - 1)) >= map.length) {
            FAIL("Bit field widths total to " + sum + ": wrong total for map of size " + map.length);
        }
        ...
        if (verbose) {
            ...
            System.out.println("A table access looks like " + genAccess("A", "ch", (identifiers ? 2 : 32)));
        }
    ...
    */
    public static void main(String[] args) {
        ...
        try {
            ...
            if (verbose && false) {
                System.out.println("Offset range seen: -" + hex8(-minOffsetSeen) + "..+" + hex8(maxOffsetSeen));
                System.out.println("          allowed: -" + hex8(-minOffset) + "..+" + hex8(maxOffset));
            }
        }
