...
public class VerifyAccess {
    ...
    public static boolean isMemberAccessible(Class<?> refc, // symbolic ref class
        ...
        if (allowedModes == 0)
            return false;
        assert((allowedModes & PUBLIC) != 0 && (allowedModes & ~(ALL_ACCESS_MODES|PACKAGE_ALLOWED|MODULE_ALLOWED|UNCONDITIONAL_ALLOWED)) == 0);
        ...
        case PROTECTED:
            ...
            if ((mods & STATIC) != 0 && !isRelatedClass(refc, lookupClass))
                return false;
            if ((allowedModes & PROTECTED) != 0 && isSubClass(lookupClass, defc))
                return true;
            return false;
        ...
        case PRIVATE:
            ...
            boolean canAccess = ((allowedModes & PRIVATE) != 0 && Reflection.areNestMates(defc, lookupClass));
    ...
    static boolean isSubClass(Class<?> lookupClass, Class<?> defc) {
        return defc.isAssignableFrom(lookupClass) && !lookupClass.isInterface(); // interfaces are types, not classes.
    }
    ...
    public static boolean isClassAccessible(Class<?> refc, Class<?> lookupClass, int allowedModes) {
        if (allowedModes == 0)
            return false;
        assert((allowedModes & PUBLIC) != 0 && (allowedModes & ~(ALL_ACCESS_MODES|PACKAGE_ALLOWED|MODULE_ALLOWED|UNCONDITIONAL_ALLOWED)) == 0);
        if (isPublic(getClassModifiers(refc)))
            return true;
        if ((allowedModes & PACKAGE_ALLOWED) != 0 && isSamePackage(lookupClass, refc))
            return true;
        return false;
    }
    ...
    public static boolean isTypeVisible(Class<?> type, Class<?> refc) {
        ...
        try {
            ...
            res = null; // Assume the class is not found
        }
        return (type == res);
    }
