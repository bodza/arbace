...
public final class TypeAnnotationParser {
     ...
     */
    public static AnnotatedType buildAnnotatedType(byte[] rawAnnotations, ConstantPool cp, AnnotatedElement decl, Class<?> container, Type type, TypeAnnotationTarget filter) {
    ...
    public static AnnotatedType[] buildAnnotatedTypes(byte[] rawAnnotations, ConstantPool cp, AnnotatedElement decl, Class<?> container, Type[] types, TypeAnnotationTarget filter) {
        ...
        if (decl instanceof Constructor) {
            ...
            if (!declaringClass.isEnum() && (declaringClass.isMemberClass() && (declaringClass.getModifiers() & Modifier.STATIC) == 0)) {
                offset = true;
            }
        }
        for (int i = 0; i < size; i++) {
            ...
            result[i] = AnnotatedTypeFactory.buildAnnotatedType(types[i], AnnotatedTypeFactory.nestingForType(types[i], LocationInfo.BASE_LOCATION), typeAnnotations, typeAnnotations, decl);
        }
        return result;
    }
    ...
    static TypeAnnotation[] parseAllTypeAnnotations(AnnotatedElement decl) {
        ...
        if (decl instanceof Class) {
            ...
            rawBytes = container.getRawTypeAnnotations();
        } else if (decl instanceof Executable) {
            container = ((Executable)decl).getDeclaringClass();
            rawBytes = Class.getExecutableTypeAnnotationBytes((Executable)decl);
        } else {
            // Should not reach here. Assert?
            return EMPTY_TYPE_ANNOTATION_ARRAY;
        }
        return parseTypeAnnotations(rawBytes, container.getConstantPool(), decl, container);
    }
    ...
    static Map<Class<? extends Annotation>, Annotation> mapTypeAnnotations(TypeAnnotation[] typeAnnos) {
        ...
        for (TypeAnnotation t : typeAnnos) {
            ...
            if (a != null) {
                ...
                if (type.retention() == RetentionPolicy.RUNTIME && result.put(klass, a) != null) {
    ...
    private static TypeAnnotationTargetInfo parseTargetInfo(ByteBuffer buf) {
        ...
        case METHOD_TYPE_PARAMETER: {
            ...
            if (posCode == CLASS_TYPE_PARAMETER)
                ...
                res = new TypeAnnotationTargetInfo(TypeAnnotationTarget.METHOD_TYPE_PARAMETER, index);
            return res;
            } // unreachable break;
        case CLASS_EXTENDS: {
            ...
            if (index == -1) {
                ...
                TypeAnnotationTargetInfo res = new TypeAnnotationTargetInfo(TypeAnnotationTarget.CLASS_IMPLEMENTS, index);
                return res;
            }} break;
