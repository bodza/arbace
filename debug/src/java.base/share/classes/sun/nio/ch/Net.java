...
public class Net {
    ...
    public static boolean isFastTcpLoopbackRequested() {
        String useFastTcpLoopback = null; // "jdk.net.useFastTcpLoopback"
        boolean enable;
        if ("".equals(useFastTcpLoopback)) {
            enable = true;
        } else {
            enable = Boolean.parseBoolean(useFastTcpLoopback);
        }
        return enable;
    }
    ...
    static {
        ...
        if (availLevel >= 0) {
            String exclBindProp = null; // "sun.net.useExclusiveBind"
