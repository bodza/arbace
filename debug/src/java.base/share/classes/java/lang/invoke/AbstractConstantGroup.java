...
abstract class AbstractConstantGroup implements ConstantGroup {
    ...
    static class SubGroup extends AbstractConstantGroup {
        ...
        public int copyConstants(int start, int end, Object[] buf, int pos) throws LinkageError {
            rangeCheck2(start, end, size);
            return self.copyConstants(offset + start, offset + end, buf, pos);
        }

        @Override
        public int copyConstants(int start, int end, Object[] buf, int pos, Object ifNotPresent) {
            rangeCheck2(start, end, size);
            return self.copyConstants(offset + start, offset + end, buf, pos, ifNotPresent);
        }
    }
    ...
    static class AsList extends AbstractList<Object> {
        ...
        @Override public List<Object> subList(int start, int end) {
            rangeCheck2(start, end, size);
            return new AsList(self, offset + start, offset + end, resolving, ifNotPresent);
        }
        ...
        @Override public <T> T[] toArray(T[] a) {
            ...
            if (resolving)
                ...
                self.copyConstants(offset, offset + size, a, 0, ifNotPresent);
            if (pad > 0)
                a[size] = null;
            return a;
        }
