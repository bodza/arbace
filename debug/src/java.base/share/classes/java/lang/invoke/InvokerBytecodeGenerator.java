...
class InvokerBytecodeGenerator {
    ...
    private static MemberName resolveFrom(String name, MethodType type, Class<?> holder) {
        ...
        return MemberName.getFactory().resolveOrNull(REF_invokeStatic, member, holder);
    }
        ...
        for (int i = lambdaForm.arity; i < lambdaForm.names.length; i++) {
            ...
            switch (intr) {
                case SELECT_ALTERNATIVE:
                    ...
                    if (PROFILE_GWT) {
                        assert(name.arguments[0] instanceof Name && ((Name)name.arguments[0]).refersTo(MethodHandleImpl.class, "profileBoolean"));
    ...
    private Name emitLoop(int pos) {
        ...
        Class<?>[] loopLocalStateTypes = Stream.of(loopClauseTypes).filter(bt -> bt != BasicType.V_TYPE).map(BasicType::basicTypeClass).toArray(Class<?>[]::new);
    ...
    private void emitLoopHandleInvoke(Name holder, int handles, int clause, Name args, boolean pushLocalState, MethodType type, Class<?>[] loopLocalStateTypes, int clauseDataSlot, int firstLoopStateSlot) {
