...
class LambdaFormEditor {
    ...
    LambdaForm spreadArgumentsForm(int pos, Class<?> arrayType, int arrayLength) {
        ...
        Name checkSpread = new Name(MethodHandleImpl.getFunction(MethodHandleImpl.NF_checkSpreadArgument), spreadParam, arrayLength);
    ...
    LambdaForm collectArgumentsForm(int pos, MethodType collectorType) {
        ...
        byte kind = (dropResult ? Transform.COLLECT_ARGS_TO_VOID : Transform.COLLECT_ARGS);
    ...
    LambdaForm collectArgumentArrayForm(int pos, MethodHandle arrayCollector) {
        ...
        Name callCombiner = new Name(new NamedFunction(arrayCollector, Intrinsic.NEW_ARRAY), (Object[]) /*...*/ newParams);
