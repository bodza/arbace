...
abstract class DelegatingMethodHandle extends MethodHandle {
    ...
    static LambdaForm makeReinvokerForm(MethodHandle target, int whichCache, Object constraint, boolean forceInline, NamedFunction getTargetFn, NamedFunction preActionFn) {
    ...
    static {
        try {
            MemberName member = new MemberName(DelegatingMethodHandle.class, "getTarget", MethodType.methodType(MethodHandle.class), REF_invokeVirtual);
            NF_getTarget = new NamedFunction(MemberName.getFactory().resolveOrFail(REF_invokeVirtual, member, DelegatingMethodHandle.class, NoSuchMethodException.class));
