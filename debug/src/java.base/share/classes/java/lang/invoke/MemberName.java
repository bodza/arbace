...
final class MemberName implements Member, Cloneable {
     ...
     *  must be a method or constructor.
     */
    /* oops! */public String getMethodDescriptor() {
        if (type == null) {
            expandFromVM();
            if (type == null) {
                return null;
    ...
    public boolean isVarHandleMethodInvoke() {
        final int bits = MH_INVOKE_MODS &~ Modifier.PUBLIC;
        final int negs = Modifier.STATIC;
        if (testFlags(bits | negs, bits) && clazz == VarHandle.class) {
    ...
    public MemberName(Method m, boolean wantSpecial) {
        ...
        if (clazz == null) { // MHN.init failed
            ...
            if (m.getDeclaringClass() == VarHandle.class && isVarHandleMethodInvokeName(m.getName())) {
                // The JVM did not reify this signature-polymorphic instance.
                // Need a special case here.
                // See comments on MethodHandleNatives.linkMethod.
                MethodType type = MethodType.methodType(m.getReturnType(), m.getParameterTypes());
    ...
    static class Factory {
         ...
         */
        private MemberName resolve(byte refKind, MemberName ref, Class<?> lookupClass, boolean speculativeResolve) {
