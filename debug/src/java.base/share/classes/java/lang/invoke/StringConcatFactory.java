...
public final class StringConcatFactory {
    ...
    static {
        ...
        final String strategy = null; // "java.lang.invoke.stringConcat"
        CACHE_ENABLE = false; // "java.lang.invoke.stringConcat.cache"
        final String dumpPath = null; // "java.lang.invoke.stringConcat.dumpClasses"
        ...
        public Recipe(String src, Object[] constants) {
            ...
            for (int i = 0; i < src.length(); i++) {
                ...
                if (c == TAG_CONST || c == TAG_ARG) {
                    ...
                    // as a constant first:
                    if (sb.length() > 0) {
                        el.add(new RecipeElement(sb.toString()));
                        sb.setLength(0);
                    }
                    if (c == TAG_CONST) {
     ...
     * </ul>
     *
     * The concatenation <em>recipe</em> is a String description for the way to
    ...
    private static CallSite doStringConcat(MethodHandles.Lookup lookup, String name, MethodType concatType, boolean generateRecipe, String recipe, Object... constants) throws StringConcatException {
        ...
        if (concatType.parameterSlotCount() > MAX_INDY_CONCAT_ARG_SLOTS) {
            throw new StringConcatException("Too many concat argument slots: " + concatType.parameterSlotCount() + ", can only accept " + MAX_INDY_CONCAT_ARG_SLOTS);
        }
    ...
    private static MethodType adaptType(MethodType args) {
        ...
        for (int i = 0; i < args.parameterCount(); i++) {
            ...
            if (!ptype.isPrimitive() && ptype != String.class && ptype != Object.class) { // truncate to Object
                if (ptypes == null) {
    ...
    private static String getClassName(Class<?> hostClass) throws StringConcatException {
         ...
         * the prefix as the cache key too. This only affects BC_* strategies, and only
     ...
     *
     * <b>{@link Strategy#BC_SB}: "bytecode StringBuilder".</b>
     *
     ...
     *
     * <b>{@link Strategy#BC_SB_SIZED}: "bytecode StringBuilder, but
     * sized".</b>
     *
     ...
     *
     * <b>{@link Strategy#BC_SB_SIZED_EXACT}: "bytecode StringBuilder, but
     * sized exactly".</b>
     *
    ...
    private static final class BytecodeStringBuilderStrategy {
        ...
        private static MethodHandle generate(Lookup lookup, String className, MethodType args, Recipe recipe, Mode mode) throws Exception {
            ...
            MethodVisitor mv = cw.visitMethod(ACC_PUBLIC + ACC_STATIC + ACC_FINAL, METHOD_NAME, args.toMethodDescriptorString(), null, null);
            ...
            if (mode.isExact()) {
                ...
                for (int c = 0; c < arr.length; c++) {
                    ...
                    if (cl == String.class) {
                        ...
                        mv.visitIntInsn(getLoadOpcode(cl), off);
                        mv.visitMethodInsn(INVOKESTATIC, "java/lang/String", "valueOf", getStringValueOfDesc(cl), false);
                        mv.visitIntInsn(ASTORE, modOff);
                        arr[c] = String.class;
     ...
     *
     * <b>{@link Strategy#MH_SB_SIZED}: "MethodHandles StringBuilder,
     * sized".</b>
     *
     ...
     *
     * <b>{@link Strategy#MH_SB_SIZED_EXACT}: "MethodHandles StringBuilder,
     * sized exactly".</b>
     *
    ...
    private static final class MethodHandleStringBuilderStrategy {
        ...
        private static MethodHandle appender(Class<?> appendType) {
            MethodHandle appender = lookupVirtual(MethodHandles.publicLookup(), StringBuilder.class, "append", StringBuilder.class, adaptToStringBuilder(appendType));
        ...
        private static final Function<Integer, MethodHandle> SUMMER = new Function<Integer, MethodHandle>() {
            ...
            public MethodHandle apply(Integer cnt) {
                if (cnt == 1) {
                    ...
                    return lookupStatic(Lookup.IMPL_LOOKUP, MethodHandleStringBuilderStrategy.class, "sum", int.class, int.class, int[].class).asCollector(int[].class, cnt - 1);
                }
            }
        };
    ...
    /**
     * <b>{@link Strategy#MH_INLINE_SIZED_EXACT}: "MethodHandles inline,
     * sized exactly".</b>
     *
    ...
    private static final class MethodHandleInlineCopyStrategy {
        ...
        private static final Function<Class<?>, MethodHandle> PREPEND = new Function<Class<?>, MethodHandle>() {
            ...
            public MethodHandle apply(Class<?> c) {
                return lookupStatic(Lookup.IMPL_LOOKUP, STRING_HELPER, "prepend", int.class, int.class, byte[].class, byte.class, Wrapper.asPrimitiveType(c));
            }
        };
        ...
        private static final Function<Class<?>, MethodHandle> CODER_MIX = new Function<Class<?>, MethodHandle>() {
            @Override
            public MethodHandle apply(Class<?> c) {
                return lookupStatic(Lookup.IMPL_LOOKUP, STRING_HELPER, "mixCoder", byte.class, byte.class, Wrapper.asPrimitiveType(c));
            }
        };
        ...
        private static final Function<Class<?>, MethodHandle> LENGTH_MIX = new Function<Class<?>, MethodHandle>() {
            @Override
            public MethodHandle apply(Class<?> c) {
                return lookupStatic(Lookup.IMPL_LOOKUP, STRING_HELPER, "mixLen", int.class, int.class, Wrapper.asPrimitiveType(c));
            }
        };
