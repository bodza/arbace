...
class DirectMethodHandle extends MethodHandle {
    ...
    private DirectMethodHandle(MethodType mtype, LambdaForm form, MemberName member) {
        ...
        if (member.getDeclaringClass().isInterface() && member.getReferenceKind() == REF_invokeInterface && member.isMethod() && !member.isAbstract()) {
    ...
    static LambdaForm makePreparedLambdaForm(MethodType mtype, int which) {
        ...
        boolean needsReceiverCheck = (which == LF_INVINTERFACE || which == LF_INVSPECIAL_IFC);
        ...
        try {
            switch (func) {
                ...
                case NF_UNSAFE:
                    ...
                    return new NamedFunction(MemberName.getFactory().resolveOrFail(REF_getField, member, DirectMethodHandle.class, NoSuchMethodException.class));
                case NF_checkReceiver:
                    member = new MemberName(DirectMethodHandle.class, "checkReceiver", OBJ_OBJ_TYPE, REF_invokeVirtual);
                    return new NamedFunction(MemberName.getFactory().resolveOrFail(REF_invokeVirtual, member, DirectMethodHandle.class, NoSuchMethodException.class));
                default:
    ...
    private static NamedFunction getNamedFunction(String name, MethodType type) throws ReflectiveOperationException
    {
        MemberName member = new MemberName(DirectMethodHandle.class, name, type, REF_invokeStatic);
        return new NamedFunction(MemberName.getFactory().resolveOrFail(REF_invokeStatic, member, DirectMethodHandle.class, NoSuchMethodException.class));
    }
