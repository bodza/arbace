...
final class StringUTF16 {
    ...
    public static String toLowerCase(String str, byte[] value) {
        int first;
        ...
        if (hasSurr) {
            return toLowerCaseEx(str, value, result, first, false);
        }
        int bits = 0;
        for (int i = first; i < len; i++) {
            ...
            if (cp == '\u03A3' ||                       // GREEK CAPITAL LETTER SIGMA
                ...
                return toLowerCaseEx(str, value, result, i, false);
            }
            if (cp == '\u0130') { // LATIN CAPITAL LETTER I WITH DOT ABOVE
                return toLowerCaseEx(str, value, result, i, true);
            }
    ...
    private static String toLowerCaseEx(String str, byte[] value, byte[] result, int first, boolean localeDependent) {
        ...
        for (int i = first; i < length; i += srcCount) {
            ...
            if (localeDependent ||
                ...
                lowerChar = Character.toLowerCase(str.codePointAt(i));
            } else {
    ...
    public static String toUpperCase(String str, byte[] value) {
        int first;
        boolean hasSurr = false;
        final int len = value.length >> 1;
        ...
        if (hasSurr) {
            return toUpperCaseEx(str, value, result, first, false);
        }
        int bits = 0;
        for (int i = first; i < len; i++) {
            int cp = (int)getChar(value, i);
            if (Character.isSurrogate((char)cp)) {
                return toUpperCaseEx(str, value, result, i, false);
            }
    ...
    private static String toUpperCaseEx(String str, byte[] value, byte[] result, int first, boolean localeDependent)
    {
        ...
        for (int i = first; i < length; i += srcCount) {
            ...
            if (localeDependent) {
                upperChar = Character.toUpperCaseEx(str.codePointAt(i));
            } else {
            ...
            if (Character.isBmpCodePoint(upperChar)) {
                ...
                if (upperChar == Character.ERROR) {
                    if (localeDependent) {
                        upperCharArray = Character.toUpperCaseCharArray(str.codePointAt(i));
                    } else {
