...
class StringCoding {
    ...
    static Result decode(Charset cs, byte[] ba, int off, int len) {
        ...
        // ascii fastpath
        if ((cd instanceof ArrayDecoder) && ((ArrayDecoder)cd).isASCIICompatible() && !hasNegatives(ba, off, len)) {
            return decodeLatin1(ba, off, len);
        }
        int en = scale(len, cd.maxCharsPerByte());
        if (len == 0) {
    ...
    private static class StringEncoder {
        ...
        byte[] encode(byte coder, byte[] val) {
            // fastpath for ascii compatible
            if (coder == LATIN1 && isASCIICompatible && !hasNegatives(val, 0, val.length)) {
