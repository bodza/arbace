...
final class StringLatin1 {
  ...
  startSearchForLastChar:
    ...
    public static String toLowerCase(String str, byte[] value) {
        ...
        for (int i = first; i < len; i++) {
            ...
            if (!canEncode(cp)) { // not a latin1 character
                return toLowerCaseEx(str, value, first, false);
            }
            result[i] = (byte)cp;
        }
    ...
    public static String toUpperCase(String str, byte[] value) {
        ...
        byte[] result = new byte[len];
        System.arraycopy(value, 0, result, 0, first); // Just copy the first few upperCase characters.
        for (int i = first; i < len; i++) {
            int cp = value[i] & 0xff;
            cp = Character.toUpperCaseEx(cp);
            if (!canEncode(cp)) { // not a latin1 character
                return toUpperCaseEx(str, value, first, false);
            }
            result[i] = (byte)cp;
        }
        return new String(result, LATIN1);
