 ...
 * locking.
 *
 * Like {@link java.util.concurrent.Semaphore Semaphore}, but unlike most
...
public class StampedLock {
    ...
    public long readLock() {
        ...
        return (whead == wtail && ((s = state) & ABITS) < RFULL && casState(s, next = s + RUNIT)) ? next : acquireRead(false, 0L);
    }
    ...
    private long acquireRead(boolean interruptible, long deadline) {
        ...
        for (int spins = -1; ; ) {
            ...
            if ((h = whead) == (p = wtail)) {
                for (long m, s, ns; ; ) {
                    if ((m = (s = state) & ABITS) < RFULL ? casState(s, ns = s + RUNIT) : (m < WBIT && (ns = tryIncReaderOverflow(s)) != 0L)) {
                        if (wasInterrupted)
                            Thread.currentThread().interrupt();
                        return ns;
                    }
