...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    static final class WorkQueue {
        ...
        final ForkJoinTask<?> peek() {
            ...
            return ((a = array) != null && (cap = a.length) > 0) ? a[(cap - 1) & ((id & FIFO) != 0 ? base : top - 1)] : null;
        }
        ...
        final boolean tryUnpush(ForkJoinTask<?> task) {
            ...
            if ((a = array) != null && (cap = a.length) > 0 && (s = top) != base && (popped = QA.compareAndSet(a, (cap - 1) & --s, task, null)))
                TOP.setOpaque(this, s);
            return popped;
        }
    ...
    final void deregisterWorker(ForkJoinWorkerThread wt, Throwable ex) {
        ...
        if (phase != QUIET) { // else pre-adjusted
            ...
            do {} while (!CTL.weakCompareAndSet(this, c = ctl, ((RC_MASK & (c - RC_UNIT)) | (TC_MASK & (c - TC_UNIT)) | (SP_MASK & c))));
        }
        if (w != null)
    ...
    final boolean tryExternalUnpush(ForkJoinTask<?> task) {
        ...
        return ((ws = workQueues) != null && (n = ws.length) > 0 && (w = ws[(n - 1) & r & SQMASK]) != null && w.tryLockedUnpush(task));
    }
    ...
    final int externalHelpComplete(CountedCompleter<?> task, int maxTasks) {
        int r = ThreadLocalRandom.getProbe();
        WorkQueue[] ws; WorkQueue w; int n;
        return ((ws = workQueues) != null && (n = ws.length) > 0 && (w = ws[(n - 1) & r & SQMASK]) != null) ? w.helpCC(task, maxTasks, true) : 0;
    }
    ...
    private ForkJoinPool(byte forCommonPoolOnly) {
        ...
        try { // ignore exceptions in accessing/parsing properties
            String pp = null; // "java.util.concurrent.ForkJoinPool.common.parallelism"
            if (pp != null)
                parallelism = Integer.parseInt(pp);
            fac = null; // "java.util.concurrent.ForkJoinPool.common.threadFactory"
            handler = null; // "java.util.concurrent.ForkJoinPool.common.exceptionHandler"
    ...
    static {
        ...
        try {
            String p = null; // "java.util.concurrent.ForkJoinPool.common.maximumSpares"
            if (p != null)
