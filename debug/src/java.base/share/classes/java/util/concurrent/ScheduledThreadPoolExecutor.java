...
public class ScheduledThreadPoolExecutor extends ThreadPoolExecutor implements ScheduledExecutorService {
    ...
    boolean canRunInCurrentRunState(RunnableScheduledFuture<?> task) {
        ...
        if (isStopped())
            return false;
        return task.isPeriodic() ? continueExistingPeriodicTasksAfterShutdown : (executeExistingDelayedTasksAfterShutdown || task.getDelay(NANOSECONDS) <= 0);
    }
    ...
    @Override void onShutdown() {
        ...
        for (Object e : q.toArray()) {
            if (e instanceof RunnableScheduledFuture) {
                ...
                if ((t.isPeriodic() ? !keepPeriodic : (!keepDelayed && t.getDelay(NANOSECONDS) > 0)) || t.isCancelled()) { // also remove if already cancelled
                    if (q.remove(t))
                        t.cancel(false);
                }
            }
     ...
     *
     * Iteration over this queue is <em>not</em> guaranteed to traverse
    ...
    static class DelayedWorkQueue extends AbstractQueue<Runnable> implements BlockingQueue<Runnable> {
        ...
        public int drainTo(Collection<? super Runnable> c, int maxElements) {
            ...
            try {
                int n = 0;
                for (RunnableScheduledFuture<?> first; n < maxElements && (first = queue[0]) != null && first.getDelay(NANOSECONDS) <= 0; ) {
