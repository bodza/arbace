 ...
 *
 * Groups beginning with {@code (?} are either pure, <i>non-capturing</i> groups
 ...
 *
 * <ul>
 *    <li>The backreference constructs, <code>\g{</code><i>n</i><code>}</code> for
 *    the <i>n</i><sup>th</sup><a href="#cg">capturing group</a> and
 *    <code>\g{</code><i>name</i><code>}</code> for
 *    <a href="#groupname">named-capturing group</a>.</li>
 *
 *    <li>The conditional constructs
 *    {@code (?(}<i>condition</i>{@code )}<i>X</i>{@code )} and
 *    {@code (?(}<i>condition</i>{@code )}<i>X</i>{@code |}<i>Y</i>{@code )},</li>
 *
 *    <li>The embedded code constructs <code>(?{</code><i>code</i><code>})</code>
 *    and <code>(??{</code><i>code</i><code>})</code>,</li>
 *
 *    <li>The embedded comment syntax {@code (?#comment)}, and</li>
 *
 *    <li>The preprocessing operations {@code \l} <code>&#92;u</code>,
 *    {@code \L}, and {@code \U}.</li>
 * </ul>
 *
 ...
 *
 * <ul>
 *    <li>In Perl, {@code \1} through {@code \9} are always interpreted
 ...
 *    smaller or equal to the existing number of groups or it is one digit.</li>
 *
 *    <li>Perl uses the {@code g} flag to request a match that resumes
...
loop:   for (int x = 0, offset = 0; x<nCodePoints; x++, offset+=len) {
    ...
    LOOP:
        for (;;) {
            ...
            if (node instanceof GreedyCharProperty && ((GreedyCharProperty)node).cp instanceof Dot) {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i < matcher.to) {
                ...
                if (ch == 0x0D) {
                    i++;
                    if (i < matcher.to) {
                        if (seq.charAt(i) == 0x0A && next.match(matcher, i + 1, seq)) {
                            return true;
                        }
                    } else {
                        matcher.hitEnd = true;
    ...
    static class CharProperty extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                ...
                return predicate.is(ch) && next.match(matcher, i + Character.charCount(ch), seq);
            } else {
                matcher.hitEnd = true;
                return false;
            }
    ...
    private static class BmpCharProperty extends CharProperty {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                return predicate.is(seq.charAt(i)) && next.match(matcher, i + 1, seq);
            } else {
                matcher.hitEnd = true;
                return false;
            }
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                ...
                if (i + n == j) { // single, assume nfc cp
                    ...
                    while (i + n < j) {
                        String nfc = Normalizer.normalize(seq.toString().substring(i, j), Normalizer.Form.NFC);
                        if (nfc.codePointCount(0, nfc.length()) == 1) {
                            if (predicate.is(nfc.codePointAt(0)) && next.match(matcher, j, seq)) {
                                return true;
                            }
                        }
    ...
    static class GraphemeBound extends Node {
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i < endIndex) {
                if (Character.isSurrogatePair(seq.charAt(i-1), seq.charAt(i)) || !Grapheme.isBoundary(Character.codePointBefore(seq, i), Character.codePointAt(seq, i))) {
                    return false;
                }
            } else {
                matcher.hitEnd = true;
