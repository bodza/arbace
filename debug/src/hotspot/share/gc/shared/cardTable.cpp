...
size_t CardTable::compute_byte_map_size() {
  assert(_guard_index == cards_required(_whole_heap.word_size()) - 1, "uninitialized, check declaration order");
...
int CardTable::find_covering_region_by_base(HeapWord* base) {
  ...
  assert(_cur_covered_regions < _max_covered_regions, "too many covered regions");
  ...
  if (new_region.word_size() != old_region.word_size()) {
    ...
    for (ri = ind + 1; ri < _cur_covered_regions; ri++) {
      if (new_end_aligned > _committed[ri].start()) {
        assert(new_end_aligned <= _committed[ri].end(), "An earlier committed region can't cover a later committed region");
    ...
    assert(index_for(new_region.last()) <  _guard_index, "The guard card will be overwritten");
    // This line commented out cleans the newly expanded region and
    // not the aligned up expanded region.
    // jbyte* const end = byte_after(new_region.last());
    jbyte* const end = (jbyte*) new_end_for_commit;
    assert((end >= byte_after(new_region.last())) || collided || guarded, "Expect to be beyond new region unless impacting another region");
