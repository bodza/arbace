...
OopStorage::Block::block_for_ptr(const OopStorage* owner, const oop* ptr) {
  ...
  for (unsigned i = 0; i < section_count; ++i, section += section_size) {
    ...
    intptr_t* candidate_owner_addr = reinterpret_cast<intptr_t*>(&candidate->_owner);
...
bool OopStorage::reduce_deferred_updates() {
  ...
  if ((_allocate_list.ctail() != NULL) && ((_allocate_list.ctail() == block) || (_allocate_list.next(*block) != NULL))) {
...
OopStorage::EntryStatus OopStorage::allocation_status(const oop* ptr) const {
  ...
  if (block != NULL) {
    ...
    if ((index < _active_array->block_count()) && (block == _active_array->at(index)) && block->contains(ptr)) {
...
OopStorage::BasicParState::BasicParState(const OopStorage* storage, uint estimated_thread_count, bool concurrent) :
  _storage(storage),
