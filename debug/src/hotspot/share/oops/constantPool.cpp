...
void ConstantPool::save_and_throw_exception(const constantPoolHandle& this_cp, int which, constantTag tag, TRAPS) {
  ...
  if (!PENDING_EXCEPTION->
    ...
    if (old_tag != error_tag && old_tag != tag.value()) {
      // MethodType doesn't change to resolved version.
...
BasicType ConstantPool::basic_type_for_constant_at(int which) {
  return tag_at(which).basic_type();
}
...
oop ConstantPool::resolve_constant_at_impl(const constantPoolHandle& this_cp, int index, int cache_index, bool* status_return, TRAPS) {
  ...
  if (cache_index >= 0) {
    ...
    oop old_result = this_cp->resolved_references()->atomic_compare_exchange_oop(cache_index, new_result, NULL);
    if (old_result == NULL) {
      return result_oop;  // was installed
    } else {
      // Return the winning thread's result.
...
int ConstantPool::find_matching_operand(int pattern_i, const constantPoolHandle& search_cp, int search_len, TRAPS) {
