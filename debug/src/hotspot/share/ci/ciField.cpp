...
ciField::ciField(ciInstanceKlass* klass, int index) : _known_to_link_with_put(NULL), _known_to_link_with_get(NULL) {
  ...
  Klass* canonical_holder = loaded_decl_holder->find_field(name, signature, &field_desc);
...
bool ciField::will_link(ciMethod* accessing_method, Bytecodes::Code bc) {
  ...
  LinkInfo link_info(_holder->get_instanceKlass(), _name->get_symbol(), _signature->get_symbol(), accessing_method->get_Method());
