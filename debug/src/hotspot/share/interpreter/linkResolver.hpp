...
class CallInfo : public StackObj {
 ...
 private:
  ...
  CallKind     _call_kind;              // kind of call (static(=bytecode static/special + others inferred), vtable, itable)
...
class LinkInfo : public StackObj {
 ...
 public:
  ...
  LinkInfo(Klass* resolved_klass, Symbol* name, Symbol* signature, Klass* current_klass, AccessCheck check_access = needs_access_check, constantTag tag = JVM_CONSTANT_Invalid) :
  ...
  LinkInfo(Klass* resolved_klass, Symbol* name, Symbol* signature, const methodHandle& current_method, AccessCheck check_access = needs_access_check, constantTag tag = JVM_CONSTANT_Invalid) :
    _resolved_klass(resolved_klass),
...
class LinkResolver: AllStatic {
 ...
 private:
  static Method* lookup_method_in_klasses(const LinkInfo& link_info, bool in_imethod_resolve);
 ...
 private:
  ...
  static void runtime_resolve_special_method(CallInfo& result, const LinkInfo& link_info, const methodHandle& resolved_method, Handle recv, TRAPS);

  static void runtime_resolve_virtual_method(CallInfo& result, const methodHandle& resolved_method, Klass* resolved_klass, Handle recv, Klass* recv_klass, bool check_null_and_abstract, TRAPS);
  static void runtime_resolve_interface_method(CallInfo& result, const methodHandle& resolved_method, Klass* resolved_klass, Handle recv, Klass* recv_klass, bool check_null_and_abstract, TRAPS);
