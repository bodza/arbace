...
class LIR_Const: public LIR_OprPtr {
 private:
  ...
  void type_check(BasicType t)   const { }
  void type_check(BasicType t1, BasicType t2)   const { }
 ...
 public:
  ...
  LIR_Const(void* p)                       { _value.set_type(T_LONG);    _value.set_jlong((jlong)p); }
...
class LIR_OprDesc: public CompilationResourceObj {
 ...
 private:
  ...
  enum OprBits {
    ...
    , non_data_bits  = kind_bits + type_bits + size_bits + destroys_bits + last_use_bits + is_fpu_stack_offset_bits + virtual_bits + is_xmm_bits
 ...
 public:
  ...
  OprType type_field_valid()               const { return (OprType)(value() & type_mask); }
  ...
  bool is_same_register(LIR_Opr opr) {
    return (is_register() && opr->is_register() && kind_field() == opr->kind_field() && (value() & no_type_mask) == (opr->value() & no_type_mask));
  }
  ...
  bool is_single_word()      const { return check_value_mask(size_mask, single_size); }
  bool is_double_word()      const { return check_value_mask(size_mask, double_size); }
  bool is_virtual_register() const { return check_value_mask(virtual_mask, virtual_mask); }
  bool is_oop_register()     const { return type_field_valid() == object_type; }
  BasicType type_register()  const { return as_BasicType(type_field_valid()); }

  bool is_last_use()         const { return (value() & last_use_mask) != 0; }
  bool is_fpu_stack_offset() const { return (value() & is_fpu_stack_offset_mask) != 0; }
  LIR_Opr make_last_use()          { return (LIR_Opr)(value() | last_use_mask); }
  LIR_Opr make_fpu_stack_offset()  { return (LIR_Opr)(value() | is_fpu_stack_offset_mask); }

  int single_stack_ix()  const { return (int)data(); }
  int double_stack_ix()  const { return (int)data(); }
  RegNr cpu_regnr()      const { return (RegNr)data(); }
  RegNr cpu_regnrLo()    const { return (RegNr)lo_reg_half(); }
  RegNr cpu_regnrHi()    const { return (RegNr)hi_reg_half(); }
  RegNr fpu_regnr()      const { return (RegNr)data(); }
  RegNr fpu_regnrLo()    const { return (RegNr)lo_reg_half(); }
  RegNr fpu_regnrHi()    const { return (RegNr)hi_reg_half(); }
  RegNr xmm_regnr()      const { return (RegNr)data(); }
  RegNr xmm_regnrLo()    const { return (RegNr)lo_reg_half(); }
  RegNr xmm_regnrHi()    const { return (RegNr)hi_reg_half(); }
  int   vreg_number()    const { return (RegNr)data(); }

  LIR_OprPtr* pointer()                    const { return (LIR_OprPtr*)this; }
...
class LIR_Op: public CompilationResourceObj {
 ...
 public:
  ...
  void set_fpu_pop_count(int count)           { _fpu_pop_count = count; }
...
class LIR_OpJavaCall: public LIR_OpCall {
 ...
 public:
  LIR_OpJavaCall(LIR_Code code, ciMethod* method, LIR_Opr receiver, LIR_Opr result, address addr, LIR_OprList* arguments, CodeEmitInfo* info)
  ...
  LIR_OpJavaCall(LIR_Code code, ciMethod* method, LIR_Opr receiver, LIR_Opr result, intptr_t vtable_offset, LIR_OprList* arguments, CodeEmitInfo* info)
...
class LIR_List: public CompilationResourceObj {
 ...
 public:
  ...
  //---------- instructions -------------
  void call_opt_virtual(ciMethod* method, LIR_Opr receiver, LIR_Opr result, address dest, LIR_OprList* arguments, CodeEmitInfo* info) {
    append(new LIR_OpJavaCall(lir_optvirtual_call, method, receiver, result, dest, arguments, info));
  }
  void call_static(ciMethod* method, LIR_Opr result, address dest, LIR_OprList* arguments, CodeEmitInfo* info) {
    append(new LIR_OpJavaCall(lir_static_call, method, LIR_OprFact::illegalOpr, result, dest, arguments, info));
  }
  void call_icvirtual(ciMethod* method, LIR_Opr receiver, LIR_Opr result, address dest, LIR_OprList* arguments, CodeEmitInfo* info) {
    append(new LIR_OpJavaCall(lir_icvirtual_call, method, receiver, result, dest, arguments, info));
  }
  void call_virtual(ciMethod* method, LIR_Opr receiver, LIR_Opr result, intptr_t vtable_offset, LIR_OprList* arguments, CodeEmitInfo* info) {
    append(new LIR_OpJavaCall(lir_virtual_call, method, receiver, result, vtable_offset, arguments, info));
  }
  void call_dynamic(ciMethod* method, LIR_Opr receiver, LIR_Opr result, address dest, LIR_OprList* arguments, CodeEmitInfo* info) {
  ...
  void metadata2reg  (Metadata* o, LIR_Opr reg)  { append(new LIR_Op1(lir_move, LIR_OprFact::metadataConst(o), reg)); }
  ...
  void cas_long(LIR_Opr addr, LIR_Opr cmp_value, LIR_Opr new_value, LIR_Opr t1, LIR_Opr t2, LIR_Opr result = LIR_OprFact::illegalOpr);
  void cas_obj(LIR_Opr addr, LIR_Opr cmp_value, LIR_Opr new_value, LIR_Opr t1, LIR_Opr t2, LIR_Opr result = LIR_OprFact::illegalOpr);
  void cas_int(LIR_Opr addr, LIR_Opr cmp_value, LIR_Opr new_value, LIR_Opr t1, LIR_Opr t2, LIR_Opr result = LIR_OprFact::illegalOpr);
  ...
  void jump(BlockBegin* block) { append(new LIR_OpBranch(lir_cond_always, T_ILLEGAL, block)); }
  void jump(CodeStub* stub)    { append(new LIR_OpBranch(lir_cond_always, T_ILLEGAL, stub)); }
  ...
  void call_runtime(address routine, LIR_Opr tmp, LIR_Opr result, LIR_OprList* arguments, CodeEmitInfo* info) {
...
class LIR_InsertionBuffer : public CompilationResourceObj {
 ...
 public:
  ...
  void init(LIR_List* lir)  { _lir = lir; _index_and_count.clear(); _ops.clear(); }
