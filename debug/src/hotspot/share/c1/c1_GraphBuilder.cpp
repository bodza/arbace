...
class BlockListBuilder {
 ...
 public:
  // creation
  BlockListBuilder(Compilation* compilation, IRScope* scope);
...
BlockListBuilder::BlockListBuilder(Compilation* compilation, IRScope* scope)
...
void BlockListBuilder::set_entries() {
...
void GraphBuilder::ScopeData::set_inline_cleanup_info(BlockBegin* block, Instruction* return_prev, ValueStack* return_state) {
...
void GraphBuilder::load_indexed(BasicType type) {
  ...
  Value array = apop();
  Value length = NULL;
  if (CSEArrayLength || (array->as_AccessField() && array->as_AccessField()->field()->is_constant()) || (array->as_NewArray() && array->as_NewArray()->length() && array->as_NewArray()->length()->type()->is_constant())) {
...
void GraphBuilder::store_indexed(BasicType type) {
  ...
  if (array_type != NULL) {
    if (array_type->is_loaded() && array_type->as_array_klass()->element_type()->basic_type() == T_BOOLEAN) {
...
void GraphBuilder::if_node(Value x, If::Condition cond, Value y, ValueStack* state_before) {
  ...
  append(new If(x, cond, false, y, tsux, fsux, is_bb ? state_before : NULL, is_bb));
}
...
void GraphBuilder::call_register_finalizer() {
  ...
  if (exact_type == NULL && receiver->as_Local() && receiver->as_Local()->java_index() == 0) {
  ...
  if (needs_check) {
    ...
    append_split(new Intrinsic(voidType, vmIntrinsics::_Object_init, state()->pop_arguments(1), true, state_before, true));
  }
}

void GraphBuilder::method_return(Value x, bool ignore_return) {
  if (RegisterFinalizersAtInit && method()->intrinsic_id() == vmIntrinsics::_Object_init) {
    call_register_finalizer();
  }

  bool need_mem_bar = false;
  if (method()->name() == ciSymbol::object_initializer_name() &&
      (scope()->wrote_final() || (AlwaysSafeConstructors && scope()->wrote_fields()) || (support_IRIW_for_not_multiple_copy_atomic_cpu && scope()->wrote_volatile())
     )) {
    need_mem_bar = true;
  }
...
Value GraphBuilder::make_constant(ciConstant field_value, ciField* field) {
  ...
  if (FoldStableValues && field->is_stable() && field_type == T_ARRAY && !field_value.is_null_or_zero()) {
...
void GraphBuilder::access_field(Bytecodes::Code code) {
  ...
  const bool needs_patching = !holder->is_loaded() || !field->will_link(method(), code);
  ...
  switch (code) {
    ...
    case Bytecodes::_getfield: {
      ...
      if (field->is_constant() && obj_type->is_constant()) {
...
void GraphBuilder::invoke(Bytecodes::Code code) {
  ...
  if (Inline && target->is_loaded() && (klass->is_initialized() || (klass->is_interface() && target->holder()->is_initialized())) && !patch_for_appendix) {
    ...
    if (code == Bytecodes::_invokestatic || code == Bytecodes::_invokespecial || (code == Bytecodes::_invokevirtual && target->is_final_method())) {
      ...
      bool success = try_inline(target, false, false, code);
  ...
  const bool has_receiver = (code == Bytecodes::_invokespecial) || (code == Bytecodes::_invokevirtual) || (code == Bytecodes::_invokeinterface);
  ...
  bool need_null_check = (code == Bytecodes::_invokespecial) || (target->is_loaded() && target->is_final_method());
...
void GraphBuilder::new_object_array() {
  bool will_link;
  ciKlass* klass = stream()->get_klass(will_link);
  ValueStack* state_before = !klass->is_loaded() ? copy_state_before() : copy_state_exhandling();
...
bool GraphBuilder::direct_compare(ciKlass* k) {
  if (k->is_loaded() && k->is_instance_klass()) {
    if (k->as_instance_klass()->is_final()) {
      return true;
    }
  }
  return false;
...
void GraphBuilder::check_cast(int klass_index) {
  bool will_link;
  ciKlass* klass = stream()->get_klass(will_link);
  ValueStack* state_before = !klass->is_loaded() ? copy_state_before() : copy_state_for_exception();
...
BlockBegin* GraphBuilder::setup_start_block(BlockBegin* std_entry, ValueStack* state) {
  ...
  if (std_entry->number_of_preds() > 0) {
  ...
  , _inline_bailout_msg(NULL)
{
  ...
  _start = setup_start_block(start_block, _initial_state);

  eliminate_redundant_phis(_start);
}

ValueStack* GraphBuilder::copy_state_before()        { return copy_state_before_with_bci(bci()); }
ValueStack* GraphBuilder::copy_state_exhandling()    { return copy_state_exhandling_with_bci(bci()); }
ValueStack* GraphBuilder::copy_state_for_exception() { return copy_state_for_exception_with_bci(bci()); }
...
bool GraphBuilder::try_inline(ciMethod* callee, bool holder_known, bool ignore_return, Bytecodes::Code bc, Value receiver) {
  ...
  // handle intrinsics
  if (callee->intrinsic_id() != vmIntrinsics::_none && (CheckIntrinsics ? callee->intrinsic_candidate() : true)) {
...
void GraphBuilder::build_graph_for_intrinsic(ciMethod* callee, bool ignore_return) {
  ...
  Intrinsic* result = new Intrinsic(result_type, callee->intrinsic_id(), args, has_receiver, state_before, vmIntrinsics::preserves_state(id), vmIntrinsics::can_trap(id));
...
bool GraphBuilder::try_inline_full(ciMethod* callee, bool holder_known, bool ignore_return, Bytecodes::Code bc, Value receiver) {
  ...
  if (callee->force_inline() || inlinee_by_directive) {
    ...
    if (callee->name() == ciSymbol::object_initializer_name() && callee->holder()->is_subclass_of(ciEnv::current()->Throwable_klass())) {
  ...
  const bool has_receiver = (bc != Bytecodes::_invokestatic);
  ...
  if (callee->is_synchronized()) {
    lock = callee->is_static() ? append(new Constant(new InstanceConstant(callee->holder()->java_mirror()))) : state()->local_at(0);
  ...
  if (num_returns() == 1 && block() == orig_block && block() == inline_cleanup_block()) {
...
void GraphBuilder::push_scope(ciMethod* callee, BlockBegin* continuation) {
  IRScope* callee_scope = new IRScope(compilation(), scope(), bci(), callee, false);
  scope()->add_callee(callee_scope);

  BlockListBuilder blb(compilation(), callee_scope);
  CHECK_BAILOUT();
